...
class AudioProcessor:
    def __init__(self, video_path=None):
        ...
        self.output_audio_path = f"temp_audio_for_{base_name}.wav"
    ...
    def _extract_audio(self):
        if os.path.exists(self.output_audio_path):
            try:
                ...
                print(f"警告：无法删除旧的临时文件 {self.output_audio_path} - {e}")
        try:
            print(f"正在使用FFmpeg从视频提取音频...")
            # 使用FFmpeg直接提取，避免MoviePy的句柄问题
            cmd = [
                'ffmpeg',
                '-i', self.video_path,
                '-vn',
            ...
            if result.returncode == 0:
                if os.path.exists(self.output_audio_path):
                    file_size = os.path.getsize(self.output_audio_path)
                    print(f"音频提取完成，文件大小: {file_size / 1024:.2f} KB")
                    return True
                else:
                    print("警告: FFmpeg执行成功但音频文件未生成")
                    return False
            else:
